...
#define IMAGE_ORIGINAL 0
#define IMAGE_ORIGINAL_V2 1
#define IMAGE_RECOVERY 2
#define IMAGE_RECOVERY_V2 3


#define CURRENT_IMAGE IMAGE_ORIGINAL

#if CURRENT_IMAGE == IMAGE_ORIGINAL
#include "bootloader_image_original.h"
#elif CURRENT_IMAGE == IMAGE_ORIGINAL_V2
#include "bootloader_image_original_v2.h"
#elif CURRENT_IMAGE == IMAGE_RECOVERY
#include "bootloader_image_recovery.h"
#elif CURRENT_IMAGE == IMAGE_RECOVERY_V2
...
void setup() {
  ...
  // 对 Leonardo/原生USB口有用；UNO 也不会卡很久
  ...
  LOGLN("\r\n[OFFLINE ISP] Power on");
  LOGLN("[OFFLINE ISP] Init OK");

  delay(300);  // 给目标板上电稳定

  LOGLN("[OFFLINE ISP] Enter program mode...");
  start_pmode();

  uint8_t s[3];
  read_sig(s);

  LOG("[OFFLINE ISP] Signature: 0x");
  LOGHEX2(s[0]); LOGHEX2(s[1]); LOGHEX2(s[2]);
  LOGLN("");

  bool ok = false;

  if (s[0]==0x1E && s[1]==0x98 && s[2]==0x01) {
    LOGLN("[OFFLINE ISP] Target = ATmega2560");
    digitalWrite(LED_PMODE, HIGH);

    ok = burn_bootloader_2560_with_log();   // 下面第3节给你
  } else {
    LOGLN("[OFFLINE ISP] ERROR: Unknown target, abort.");
  }

  LOGLN("[OFFLINE ISP] Leave program mode");
  end_pmode();

  if (ok) {
    LOGLN("[OFFLINE ISP] SUCCESS");
    while (1) {
      digitalWrite(13, HIGH); delay(1000);
      digitalWrite(13, LOW);  delay(1000);
    }
  } else {
    LOGLN("[OFFLINE ISP] FAIL");
    while (1) {
...
// ===== Offline ISP burn bootloader for ATmega2560 =====
static const uint8_t SIG_2560[3] = { 0x1E, 0x98, 0x01 };

// 读签名字节
static void read_sig(uint8_t sig[3]) {
  sig[0] = spi_transaction(0x30, 0x00, 0x00, 0x00);
  sig[1] = spi_transaction(0x30, 0x00, 0x01, 0x00);
  sig[2] = spi_transaction(0x30, 0x00, 0x02, 0x00);
}

static bool sig_is_2560() {
  uint8_t s[3];
  read_sig(s);
  return (s[0]==SIG_2560[0] && s[1]==SIG_2560[1] && s[2]==SIG_2560[2]);
}
...
// 写 fuse（可选：你得确认你板子的晶振/配置）
static void write_lfuse(uint8_t v){ spi_transaction(0xAC, 0xA0, 0x00, v); delay(10); }
static void write_hfuse(uint8_t v){ spi_transaction(0xAC, 0xA8, 0x00, v); delay(10); }
static void write_efuse(uint8_t v){ spi_transaction(0xAC, 0xA4, 0x00, v); delay(10); }

// 写 lock bits（可选）
static void write_lock(uint8_t v){ spi_transaction(0xAC, 0xE0, 0x00, v); delay(10); }

// 读取 flash word（用于校验）
static uint16_t read_flash_word(uint32_t word_addr) {
  // word_addr 是“字地址”
  uint8_t low  = spi_transaction(0x20, (word_addr >> 8) & 0xFF, word_addr & 0xFF, 0);
  uint8_t high = spi_transaction(0x28, (word_addr >> 8) & 0xFF, word_addr & 0xFF, 0);
  return (uint16_t)high << 8 | low;
}

static bool burn_bootloader_2560_with_log() {
  const uint32_t FLASH_BYTES = 0x40000UL;
  const uint32_t BOOT_BYTES  = 0x2000UL;               // 8KB boot
  const uint32_t BOOT_START_BYTE = FLASH_BYTES - BOOT_BYTES; // 0x3E000
  const uint32_t start_word = BOOT_START_BYTE / 2;

  LOGLN("[OFFLINE ISP] Start burning bootloader...");
  LOG("[OFFLINE ISP] boot_bin_len="); SERIAL.println(boot_bin_len);
  LOG("[OFFLINE ISP] BOOT_START=0x"); SERIAL.println(BOOT_START_BYTE, HEX);

  if (boot_bin_len > BOOT_BYTES) {
  ...
  // Mega2560 flash page size = 256 bytes
  param.pagesize = 256;
  ...
  // 每写 N 字节打印一次进度（避免太频繁）
  const uint32_t PROG_STEP = 512; // bytes
  uint32_t next_report = PROG_STEP;

  while (i < boot_bin_len) {
    if (page != current_page()) {
      commit(page);
      page = current_page();
    }

    uint8_t low  = pgm_read_byte(&boot_bin[i++]);
    uint8_t high = 0xFF;
    if (i < boot_bin_len) high = pgm_read_byte(&boot_bin[i++]);

    flash(LOW,  here, low);
    flash(HIGH, here, high);
    here++;

    if (i >= next_report || i == boot_bin_len) {
      LOG("[OFFLINE ISP] Writing... ");
      SERIAL.print((i * 100UL) / boot_bin_len);
      LOGLN("%");
  ...
  for (uint16_t w = 0; w < 64; w++) {
    ...
    uint8_t low  = pgm_read_byte(&boot_bin[w*2]);
    uint8_t high = pgm_read_byte(&boot_bin[w*2 + 1]);
    uint16_t exp = ((uint16_t)high << 8) | low;
    if (got != exp) {
      LOG("[OFFLINE ISP] VERIFY FAIL at word "); SERIAL.println(w);
      LOG("[OFFLINE ISP] got=0x"); SERIAL.println(got, HEX);
      LOG("[OFFLINE ISP] exp=0x"); SERIAL.println(exp, HEX);
      return false;
    }
  }
  LOGLN("[OFFLINE ISP] Verify OK");

  // 先不写 fuse/lock（避免把板子写死）；等你确认 boot size/fuse 再加
  return true;
}
