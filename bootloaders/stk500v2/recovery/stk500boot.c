...
// ====== [修改 1] 添加 EEPROM 计数定义 ======
// 选择 EEPROM 地址 4 作为计数存储位 (避开地址0，因为掉电容易损坏)
#define EEPROM_COUNTER_ADDR  ((uint8_t*)0x04)
#define MAX_BOOT_COUNTS      5

// ===== EEPROM 清除跳线（Mega2560：D22 = PA0）=====
#define CLR_EE_DDR   DDRA
#define CLR_EE_PORT  PORTA
#define CLR_EE_PINR  PINA
#define CLR_EE_BIT   0    // PA0 = D22

#define COREBEEP_DDR   DDRB
#define COREBEEP_PORT PORTB
#define COREBEEP_BIT  4      // PB4 = Arduino D10

// ====== BEEP on Arduino D9 (Mega2560: PH6) ======
#define BEEP_DDR   DDRH
#define BEEP_PORT  PORTH
#define BEEP_PINR  PINH
#define BEEP_BIT   6      // PH6 = D9

// 如果你的蜂鸣器是“高电平响”
#define BEEP_ON()   (BEEP_PORT |=  (1 << BEEP_BIT))
#define BEEP_OFF()  (BEEP_PORT &= ~(1 << BEEP_BIT))
...
typedef enum {
    CLR_IDLE = 0,
    CLR_COUNTING,
    CLR_TRIGGERED_WAIT_RELEASE
} clr_state_t;

static unsigned char gHasFlashed;
static unsigned char gInterceptAppJump;
static uint8_t current_boot_count;
...
// static void send_uint8(uint8_t v)
// {
//     char buf[4];              // 0..255 最多 3 位 + '\0'
//     utoa(v, buf, 10);         // 转十进制字符串
//     sendstring(buf);
// }


static uint8_t is_clear_eeprom_requested_sm(void)
{
    static clr_state_t st = CLR_IDLE;
    static uint16_t low_cnt = 0;

    // input + pull-up（每次调用做一次也没问题；想省可以只初始化一次）
    CLR_EE_DDR  &= ~(1 << CLR_EE_BIT);
    CLR_EE_PORT |=  (1 << CLR_EE_BIT);

    // 读引脚：1=高，0=低（接地）
    uint8_t pin_low = ((CLR_EE_PINR & (1 << CLR_EE_BIT)) == 0);

    // 阈值：连续低电平多少次才算“稳定按下”
    // 你循环里 delay_ms(500)，那一次循环很慢；
    // 如果你后面把主循环改快了，这个阈值再调大即可。
    const uint16_t THRESH = 50;
    ...
    switch (st)
    {
        ...
        case CLR_COUNTING:
            if (pin_low) {
                if (low_cnt < 0xFFFF) low_cnt++;
                if (low_cnt >= THRESH) {
                    st = CLR_TRIGGERED_WAIT_RELEASE;
                    return 1;   // 触发一次
                }
            } else {
                st = CLR_IDLE; // 抖回高电平，重新来
            }
            break;

        case CLR_TRIGGERED_WAIT_RELEASE:
            // 等用户松开跳线（回到高电平）再允许下次触发
            if (!pin_low) {
                st = CLR_IDLE;
            }
            break;
...
static inline void corebeep_init(void)
{
    COREBEEP_DDR |= (1 << COREBEEP_BIT);      // output
    COREBEEP_PORT &= ~(1 << COREBEEP_BIT);    // default OFF
}

static inline void corebeep_on(void)
{
    COREBEEP_PORT |= (1 << COREBEEP_BIT);
}

static inline void corebeep_off(void)
{
    COREBEEP_PORT &= ~(1 << COREBEEP_BIT);
}
...
{
    // 关中断，避免带着 bootloader 状态进 App
    cli();

    // 可选：关 UART（避免 App 初始化前串口状态干扰）
    UART_CONTROL_REG = 0;

    // 可选：清一些可能影响 App 的状态（按需）
    // UART_STATUS_REG = 0;

    // 跳到 0x0000（App reset vector）
    asm volatile(
        "clr r30\n\t"
        "clr r31\n\t"
        "ijmp\n\t"
...
void user_code(void)
{
    ...
    while (1)
    {
		if (time % 50 == 0) {
			// 1) 打印当前计数
			// sendstring("[RECOVERY MODE]\r\n");
			// sendstring("Boot counter = ");
			// send_uint8(current_boot_count);
			// sendstring("\r\n");
			// sendstring("To reset counter and start application:\r\n");
			// sendstring("Short D22 to GND, then release.\r\n\r\n");
		}

        // 2) 检测 D22 跳线（无 delay 状态机）
        if (is_clear_eeprom_requested_sm())
        {
            sendstring("EEPROM CLEAR...\r\n");

            // 只清计数（你要全清 EEPROM 就换成 clear_all）
            eeprom_write_byte(EEPROM_COUNTER_ADDR, 0);

            sendstring("EEPROM CLEARED, JUMP APP\r\n");
...
//	PROGLED_PORT	&=	~(1<<PROGLED_PIN);	// active low LED ON
	...
	current_boot_count = eeprom_read_byte(EEPROM_COUNTER_ADDR);
	if (current_boot_count == 0xFF) {
		current_boot_count = 0;
		eeprom_write_byte(EEPROM_COUNTER_ADDR, 0);
	}

	if (current_boot_count >= MAX_BOOT_COUNTS)
	{
		gInterceptAppJump = 1;
	}
		...
		while (!isLeave)
		{
			...
			switch (msgBuffer[0])
			{
				...
				case CMD_LEAVE_PROGMODE_ISP:
					if (gHasFlashed)
					{
						uint8_t cnt = eeprom_read_byte(EEPROM_COUNTER_ADDR);
						if (cnt == 0xFF) cnt = 0;

						if (cnt < MAX_BOOT_COUNTS) {
							cnt++;
							eeprom_write_byte(EEPROM_COUNTER_ADDR, cnt);
						}
					}
					isLeave = 1;
					//*	fall thru
