...
**********************************************************************/
void _Drive::LED_Data_Zigbee(uint8_t One, uint8_t Two, uint8_t Three, uint8_t rank)
{
    Zigbee[0] = 0x55;
    Zigbee[1] = 0x04;
...
/**********************************************************************
 * 函 数 名 ：  控制标志物播报指定文本信息(bcp)
 * 参    数 ：  *p  --> 需要发送的数据
 * 返 回 值 ：  无
 * 简    例 ：  YY_Play_Zigbee_2("北京欢迎您");
**********************************************************************/
void  YY_Play_Zigbee_2(char* p)
{
    int n = 0;
    int len = strlen(p);     // 文本长度
    Serial.print(len);
    char temp[3];
    uint8_t temp2[len];
    uint16_t GBK;
    ...
    YY_Init[0] = 0x55;
    YY_Init[1] = 0xff & ((len + 2) >> 8); // 数据区长度高八位
    YY_Init[2] = 0xff & (len + 2);        // 数据区长度低八位
    YY_Init[3] = 0x01;                      //命令字
    YY_Init[4] = 0x01;                      //GBK编码指令
    //YY_Init[5] = temp2;                     //待合成文本
  
    Send_ZigbeeData_To_Fifo(YY_Init, 5);
    Send_ZigbeeData_To_Fifo(temp2, len);
    Serial.println("");
    for (int i = 0; i < 5; i++)
    {
        Serial.print(YY_Init[i], HEX);
        Serial.print(",");
    }
    Serial.println("");
    for (int i = 0; i < len; i++)
    ...
    ExtSRAMInterface.ExMem_Write_Bytes(0x6008, temp2, len);
    delay(100);
    YYBB[0] = YY_Init[0];
    YYBB[1] = YY_Init[1];
    YYBB[2] = YY_Init[2];
    ...
    Send_ZigbeeData_To_Fifo(YYBB, 15);
    ExtSRAMInterface.ExMem_Write_Bytes(0x6008, YYBB, 15);
}
